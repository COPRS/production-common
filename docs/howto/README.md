...
This sections are giving an overview about how to create custom RS Add-ons for the COPRS environment. Basically there are two activities that needs to be performed. A docker image needs to be created that is based on a image with the CFI. This image is expected to contain a self-contained environment that allows the execution of the worker. So it needs to contain all the binaries, libraries and configuration that are required in order to execute the processor. On this image another layer needs to be deployed that contains the COPRS Execution Worker that invokes the processor from the workflows and monitor its outcome.
...
The first step is to ensure that the image containing the CFI is available for your local docker instance. This can be eiher archieved by referencing a docker registry (like COPRS artifactory). In this case you need to get an account and login credentials in order to login before being able to access the registry. If you want to build the images locally, you can import it into your local docker engine without using a remote registry.

In this case you can simply download the docker images from the [COPRS function 1 environment](https://sentinelprocessors.copernicus.eu/) and put it into a local directory. You can then use docker to import the tarball into your local engine by using the following commands. In this example we are using teh S3 SR1 image:
...
The next step is to use the base image from the previous section and add the COPRS execution Worker to the image. This is a crucial step as this microservices is used by the workflow in order to invoke the processing and monitor the operation as well as collecting the produced artifacts and ensure that they are uploaded into the OBS and other systems are triggered to extract the metadata from it.

Keep in mind that the Execution Worker does have a few own dependencies. As a Java application it will require a JVM 11 installation within the base image. It is assumed that this is already contained in the CFI image. If this is not the case, you needs to install it either manually or use the operating system repositories.

You can find [here](https://github.com/COPRS/production-common/tree/develop/rs-container/docker_template/Dockerfile) a template for a typical creation of the images. The idea behind it is simply that the CFI image is used and the Execution Worker copied into the new image. The example provided in the template is best practise and included the creation of working directories and the usage of the tini init wrapper that will ensure that no zombie processes are accidently left behind and causing a higher hardware consumption over the time.

If you need more examples on how to build these containers, you can find [here](https://github.com/COPRS/production-common/tree/develop/rs-container/) more examples of IPFs that are supported by the COPRS environment and how they was build. Keep in mind that if you use a local docker image to build everything, you might adjust the FROM line to reference the image not from a remote image.
...
The folder `content` does contain the configuration for the RS add-on and defines the payload of the RS add-on. The following files are existing:
...
The parameter properties is a property file that contains the configuration for all the SCDF applications. The required parameter will depend on the IPF that you are trying to integrated. Please follow the User Manual of the CFI in order to determinate what needs to be setup. You can find an overview regarding the configuration on this [page](https://github.com/COPRS/processing-sentinel-3/tree/develop/docs/common).
