...
public class MqiCompressedJobController extends GenericMqiController<CompressionJobDto> {
	 ...
	 */
	@Autowired
	public MqiCompressedJobController(final MqiMessageService mongoDBServices,
			@Value("${mqi.max-retries}") final int maxRetries, final AppStatus appStatus,
			@Value("${mqi.dft-offset}") final int dftOffset) {
		super(mongoDBServices, maxRetries, ProductCategory.COMPRESSED_PRODUCTS, appStatus, dftOffset);
	}
