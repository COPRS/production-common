 ...
 */
@RestController
@RequestMapping(path = "/messages/level_segments")
public class LevelSegmentDistributionController
        extends GenericMessageDistribution<LevelSegmentDto> {
     ...
     */
    @Autowired
    public LevelSegmentDistributionController(
    ...
    public ResponseEntity<GenericMessageDto<LevelSegmentDto>> next() {
        return super.next();
    }
    ...
    public ResponseEntity<Void> publish(
            @RequestBody() final GenericPublicationMessageDto<LevelSegmentDto> message) {
        String log = String.format("[productName: %s]",
                message.getMessageToPublish().getName());
