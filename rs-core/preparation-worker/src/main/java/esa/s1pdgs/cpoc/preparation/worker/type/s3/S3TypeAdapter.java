...
import esa.s1pdgs.cpoc.preparation.worker.config.type.S3TypeAdapterProperties;
...
import esa.s1pdgs.cpoc.preparation.worker.model.tasktable.TaskTableFactory;
...
public class S3TypeAdapter extends AbstractProductTypeAdapter implements ProductTypeAdapter {
	...
	private S3TypeAdapterProperties settings;

	public S3TypeAdapter(final MetadataClient metadataClient, final TaskTableFactory ttFactory,
			final ElementMapper elementMapper, final ProcessProperties processSettings,
			final PreparationWorkerProperties workerSettings, final S3TypeAdapterProperties settings) {
	...
	public List<AppDataJob> createAppDataJobs(IpfPreparationJob job) {
		...
		if (settings.isRangeSearchActiveForProductType(ttAdapter.taskTable().getProcessorName(), productType)) {
			S3TypeAdapterProperties.RangeCoverSettings rangeSettings = settings.getRangeSearch()
