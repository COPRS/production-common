...
listtopicfile="$dirbash/topic_partition.txt"
...
do
        ...
        if [ -z "${partcount}" ] ; then
                ...
                if [ $partcount != $partitions ]; then
                        ...
                        ModifTopic=1
                       #kubectl -n $KAFKA_NAMESPACE exec -ti $KAFKA_POD -- sh /opt/bitnami/kafka/bin/kafka-topics.sh --delete --topic $topic --zookeeper $ZOOKEEPER_LIST
                fi
        fi
...
#kubectl -n $KAFKA_NAMESPACE exec -ti $KAFKA_POD -- sh /opt/bitnami/kafka/bin/kafka-topics.sh --create --topic $topic --zookeeper $ZOOKEEPER_LIST --replication-factor $replicas --partitions $partitions
        #fi
