...
public class LocalDirectoryCleaning {
	...
	/**
	 * if file.lastmodified <= curretn milliseconds - min age AND dir is empty => we can delete it
	 */
	 ...
	 */
	@Autowired
	public LocalDirectoryCleaning(@Value("${file.session-files.local-directory}") final String sessionDir,
	...
	/**
	 * If possible, clean a directory or its su-directories when they have not been
	 * modified since minAge
	 * 
	 * @param directory
	 * @param minAge
	 */
		...
		try {
			...
			if (files != null) {
				for (File child : files) {
					this.recursiveEmptyDirectoriesDeletion(child, lastModifiedMin);
				}
			}
		} catch (SecurityException e) {
			LOGGER.error("[code {}] [directory {}] [age {}] [msg {}]", ErrorCode.INGESTOR_CLEAN.getCode(), directory,
					minAge, e.getMessage());
		}
	}

	/**
	 * Recursively remove a directory and its sub-directories if they have not been
	 * modified since min age
	 * 
	 * @param file
	 ...
	 */
	protected long recursiveEmptyDirectoriesDeletion(final File file, final long lastModifiedMin) throws SecurityException {
		long nbFiles = 0;
			...
			if (files != null) {
				for (File child : files) {
					if (child.isDirectory()) {
						nbFiles += recursiveEmptyDirectoriesDeletion(child, lastModifiedMin);
					} else {
						nbFiles += 1;
					}
				}
