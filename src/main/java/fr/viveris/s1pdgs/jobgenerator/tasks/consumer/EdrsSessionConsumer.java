...
public class EdrsSessionConsumer {
    ...
    public void consumeMessages() {
        ...
        EdrsSessionDto leveldto = message.getBody();
            ...
            step = 1;
            LOGGER.info("[MONITOR] [step {}] [productName {}] Building product", step, leveldto.getObjectStorageKey());
            ...
            if (cachedSessions.containsKey(file.getSessionId())) {
                ...
                if (this.cachedSessions.size() < this.maxNbSessions) {
                    session = new EdrsSessionProduct(file.getSessionId(), leveldto.getSatelliteId(), leveldto.getMissionId(),
                            file.getStartTime(), file.getStopTime(), new EdrsSession());
                    session.getObject().setChannel(file, leveldto.getChannelId());
                    cachedSessions.put(file.getSessionId(), session);
                } else {
                    throw new JobGenMaxNumberCachedSessionsReachException("Maximal number of cached sessions reached");
                }
            }

        } catch (JobGenMaxNumberCachedSessionsReachException | JobGenMaxNumberCachedJobsReachException | ObsException mnce) {
            LOGGER.error("[MONITOR] [step {}] [productName {}] [resuming {}] [code {}] {} ", step,
                    leveldto.getObjectStorageKey(), mnce.getCode().getCode(),
                    mnce.getLogMessage());
