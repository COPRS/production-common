...
import esa.s1pdgs.cpoc.ingestor.kafka.AbstractKafkaService;
...
public class AbstractKafkaServiceTest {
	...
	public void init() {
		...
		doReturn(sendResult).when(kafkaTemplate).send(Mockito.anyString(), Mockito.any());
	 ...
	 */
	@Test
	public void testSendOk() throws InterruptedException, ExecutionException, KafkaSendException {
		doReturn(null).when(sendResult).get();

		service.send("string to send");
		Mockito.verify(kafkaTemplate, Mockito.times(1)).send(Mockito.eq(TOPIC_NAME), Mockito.eq("string to send"));
	 ...
	 */
	@Test
	public void testSendWhenCancellationException()
			throws InterruptedException, ExecutionException, KafkaSendException {
		doThrow(new CancellationException("cancel exception raised")).when(sendResult).get();
	 ...
	 */
	@Test
	public void testSendWhenInterruptedException() throws InterruptedException, ExecutionException, KafkaSendException {
		doThrow(new InterruptedException("cancel exception raised")).when(sendResult).get();
	 ...
	 */
	@Test
	public void testSendWhenExecutionException() throws InterruptedException, ExecutionException, KafkaSendException {
...
class KafkaServiceImpl extends AbstractKafkaService<String> {
	 ...
	 * @param kafkaTopic
	 */
	public KafkaServiceImpl(final KafkaTemplate<String, String> kafkaTemplate, final String kafkaTopic) {
