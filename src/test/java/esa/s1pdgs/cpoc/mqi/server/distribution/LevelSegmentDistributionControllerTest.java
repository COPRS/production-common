 ...
 */
public class LevelSegmentDistributionControllerTest extends RestControllerTest {
     ...
     */
    private GenericMessageDto<LevelSegmentDto> consumedMessage;

    /**
     * The controller to test
     */
    private LevelSegmentDistributionController controller;
    ...
    public void init() throws AbstractCodedException {
        ...
        consumedMessage =
                new GenericMessageDto<LevelSegmentDto>(123, "input-key", dto);

        doReturn(consumedMessage).when(messages).nextMessage(Mockito.any());

        doReturn(1000).when(properties).getWaitNextMs();

        controller = new LevelSegmentDistributionController(messages,
    ...
    public void testNextMessageUri() throws Exception {
        request(get("/messages/level_segments/next"))
                ...
                .andExpect(jsonPath("$.body.keyObs", is("key-obs")))
                .andExpect(jsonPath("$.body.name", is("product-name")))
                .andExpect(jsonPath("$.body.family", is("L0_SEGMENT")))
                .andExpect(jsonPath("$.body.mode", is("NRT")));
        verify(messages, times(1))
                .nextMessage(Mockito.eq(ProductCategory.LEVEL_SEGMENTS));
    }
    ...
    public void testAckMessageUri() throws Exception {
        ...
        request(post("/messages/level_segments/ack")
                .contentType(MediaType.APPLICATION_JSON_VALUE).content(dto1))
                        .andExpect(MockMvcResultMatchers.status().isOk())
                        .andExpect(content().string("true"));
        verify(messages, times(1)).ackMessage(
                Mockito.eq(ProductCategory.LEVEL_SEGMENTS), Mockito.eq(123L),
        ...
        request(post("/messages/level_segments/ack")
                .contentType(MediaType.APPLICATION_JSON_VALUE).content(dto2))
                        ...
                        .andExpect(content().string("true"));
        verify(messages, times(1)).ackMessage(
                Mockito.eq(ProductCategory.LEVEL_SEGMENTS), Mockito.eq(321L),
    ...
    public void testPublishMessageUri() throws Exception {
        ...
        request(post("/messages/level_segments/publish")
