...
public class MqiLevelJobControllerTest extends RestControllerTest {
    ...
    public void testReadMessageMqiMessageDontExists() throws Exception {
        ...
        MqiGenericReadMessageDto<LevelJobDto> body =
                new MqiGenericReadMessageDto<LevelJobDto>("group", "readingPod",
                        false, null);
    ...
    public void testNextMessageMqiMessageDontExists() throws Exception {
        ...
        request(get("/mqi/level_jobs/next").param("pod", "readingPod"))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(content()
    ...
    public void testSendMessageMqiMessageACKOK() throws Exception {
        List<MqiMessage> response = new ArrayList<MqiMessage>();
        response.add(new MqiMessage(ProductCategory.LEVEL_JOBS, "topic", 1, 5,
                "group", MqiStateMessageEnum.ACK_OK, "readingPod", null,
                "sendingPod", null, null, 2, null));
    ...
    public void testSendMessageMqiMessageRetry() throws Exception {
        List<MqiMessage> response = new ArrayList<MqiMessage>();
        response.add(new MqiMessage(ProductCategory.LEVEL_JOBS, "topic", 1, 5,
                "group", MqiStateMessageEnum.SEND, "readingPod", null,
                "sendingPod", null, null, 0, null));
        this.mockSearchByID(response);
        MqiSendMessageDto body = new MqiSendMessageDto("pod", false);
        request(post("/mqi/level_jobs/1/send")
    ...
    public void testEarliestOffsetMessageMqiMessageDontExists()
        ...
        request(get("/mqi/level_jobs/topic/1/earliestOffset").param("group",
