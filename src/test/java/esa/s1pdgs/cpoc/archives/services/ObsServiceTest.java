...
public class ObsServiceTest {
    ...
    public void init() throws ObsServiceException, SdkClientException {
        ...
        doReturn(true).when(client).doesPrefixExist(Mockito
                .eq(new ObsObject("test-key", ObsFamily.AUXILIARY_FILE)));
        doReturn(false).when(client).doesPrefixExist(
    ...
    public void testGetObsFamily() {
        ...
        assertEquals(ObsFamily.UNKNOWN,
                service.getObsFamily(ProductFamily.L0_JOB));
    }
    ...
    public void testNominalExist()
        ...
        assertTrue(ret);
        verify(client, times(1)).doesPrefixExist(Mockito
                .eq(new ObsObject("test-key", ObsFamily.AUXILIARY_FILE)));

        ret = service.exist(ProductFamily.EDRS_SESSION, "test-key");
        assertFalse(ret);
        verify(client, times(1)).doesPrefixExist(
                Mockito.eq(new ObsObject("test-key", ObsFamily.EDRS_SESSION)));
    }
    ...
    public void testDownloadFileWhenException()
        ...
        service.downloadFile(ProductFamily.AUXILIARY_FILE, "error-key",
                "test/");
    }
    ...
    public void testDownloadFileWhenException2()
        ...
        service.downloadFile(ProductFamily.EDRS_SESSION, "error-key",
                "test/");
    }
     ...
     */
    @Test
    public void testNominalDownload() throws ObsException, ObsServiceException,
            SdkClientException, ObsUnknownObject {
        File upload1 = service.downloadFile(ProductFamily.EDRS_SESSION,
                "test-key", "test/");
        verify(client, times(1))
                .downloadObject(Mockito.eq(new ObsDownloadObject("test-key",
                        ObsFamily.EDRS_SESSION, "test/")));
        ...
        File upload2 = service.downloadFile(ProductFamily.EDRS_SESSION,
                "test-key/key2", "test/");
