...
import esa.s1pdgs.cpoc.mdcatalog.controllers.kafka.L0AcnsConsumer;
...
public class L0AcnsConsumerTest {
	 ...
	 * Controller to test
	 */
	private L0AcnsConsumer controller;
	...
	public void init() throws Exception {
		...
		doAnswer(i -> {
			...
			return desc;
		}).when(mdBuilder).buildL0AcnOutputFileMetadata(Mockito.any(), Mockito.any());
		...
		controller = new L0AcnsConsumer(esServices, s3Services, LOCAL_DIRECTORY, extractorConfig, fileDescriptorBuilder,
				mdBuilder, "manifest.safe", ".safe", TOPIC_NAME);
	}
	...
	public void testReceiveWhenNotExistInOBS() throws ObjectStorageException, ObsUnknownObjectException {
		doThrow(new ObsUnknownObjectException(ProductFamily.L0_ACN, "key-obs")).when(s3Services)
				.downloadFile(Mockito.any(), Mockito.anyString(), Mockito.anyString());

		controller.receive(new KafkaL0AcnDto("product-name", "key-obs"));
		verify(s3Services, times(1)).downloadFile(Mockito.eq(ProductFamily.L0_ACN), Mockito.eq("key-obs"),
	...
	public void testReceiveWhenGetFromObsFailed() throws ObjectStorageException, ObsUnknownObjectException {
		doThrow(new ObjectStorageException(ProductFamily.L0_ACN, "key-obs", new Exception())).when(s3Services)
				.downloadFile(Mockito.any(), Mockito.anyString(), Mockito.anyString());
	...
	public void testReceiveWhenBuildDescriptorFailed()
		...
		controller.receive(new KafkaL0AcnDto("product-name", "file_no_safe.xml"));
		verify(s3Services, times(1)).downloadFile(Mockito.eq(ProductFamily.L0_ACN), Mockito.eq("file_no_safe.xml"),
				Mockito.eq(LOCAL_DIRECTORY));
	...
	public void testReceiveWhenExtractionFailed() throws ObjectStorageException, FilePathException,
		...
		doThrow(new MetadataExtractionException("product-name", new Exception("erro"))).when(mdBuilder)
				.buildL0AcnOutputFileMetadata(Mockito.any(), Mockito.any());
		L0OutputFileDescriptor desc = new L0OutputFileDescriptor();
		...
		verify(s3Services, times(1)).downloadFile(Mockito.eq(ProductFamily.L0_ACN), Mockito.eq("file_no_safe.xml"),
				Mockito.eq(LOCAL_DIRECTORY));
		verify(fileDescriptorBuilder, times(1)).buildL0OutputFileDescriptor(Mockito.eq(fileNoSafe));
		verify(mdBuilder, times(1)).buildL0AcnOutputFileMetadata(Mockito.eq(desc), Mockito.eq(fileNoSafe));
		verifyZeroInteractions(esServices);
		assertFalse(fileNoSafe.exists());
	}
	...
	public void testReceiveWhenMetadataCreationFailed() throws Exception {
		...
		verify(s3Services, times(1)).downloadFile(Mockito.eq(ProductFamily.L0_ACN), Mockito.eq("file_no_safe.xml"),
				Mockito.eq(LOCAL_DIRECTORY));
		verify(fileDescriptorBuilder, times(1)).buildL0OutputFileDescriptor(Mockito.eq(fileNoSafe));
		verify(mdBuilder, times(1)).buildL0AcnOutputFileMetadata(Mockito.any(), Mockito.eq(fileNoSafe));
		verify(esServices, times(1)).isMetadataExist(Mockito.any());
	...
	public void testReceiveSafe() throws Exception {
		...
		controller.receive(new KafkaL0AcnDto("product-name", "file_safe.safe"));
		verify(s3Services, times(1)).downloadFile(Mockito.eq(ProductFamily.L0_ACN),
				Mockito.eq("file_safe.safe/manifest.safe"),
				Mockito.eq(LOCAL_DIRECTORY));
		verify(fileDescriptorBuilder, times(1)).buildL0OutputFileDescriptor(Mockito.eq(fileSafe));
		verify(mdBuilder, times(1)).buildL0AcnOutputFileMetadata(Mockito.eq(desc), Mockito.eq(fileSafe));
		verify(esServices, times(1)).isMetadataExist(Mockito.any());
		verify(esServices, times(1)).createMetadata(Mockito.any());
