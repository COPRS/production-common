...
public class OutputProducerFactoryTest {
    ...
    public void init() throws KafkaSendException {
        ...
        this.outputProcuderFactory = new OutputProcuderFactory(senderProducts,
                senderAcns, senderL0Reports, senderL1Products, senderL1Acns,
                senderL1Reports);
    }
    ...
    public void testSendReport() throws AbstractCodedException {
        this.outputProcuderFactory
                .sendOutput(new FileQueueMessage(ProductFamily.L0_REPORT,
        ...
        verify(this.senderL0Reports, times(1))
                .send(Mockito.eq(new ReportDto("test.txt", "Test report file",
    ...
    public void testSendProduct() throws AbstractCodedException {
        this.outputProcuderFactory.sendOutput(new ObsQueueMessage(
                ProductFamily.L0_PRODUCT, "test.txt", "test.txt"));
        verify(this.senderProducts, times(1))
                .send(Mockito.eq(new L0SliceDto("test.txt", "test.txt")));
        verify(this.senderAcns, never()).send(Mockito.any());
    ...
    public void testSendAcn() throws AbstractCodedException {
        this.outputProcuderFactory.sendOutput(new ObsQueueMessage(
                ProductFamily.L0_ACN, "test.txt", "test.txt"));
        verify(this.senderAcns, times(1))
                .send(Mockito.eq(new L0AcnDto("test.txt", "test.txt")));
        verify(this.senderProducts, never()).send(Mockito.any());
        verify(this.senderL0Reports, never()).send(Mockito.any());
        verify(this.senderL1Products, never()).send(Mockito.any());
        verify(this.senderL1Acns, never()).send(Mockito.any());
    ...
    public void testSendL1Report() throws AbstractCodedException {
        this.outputProcuderFactory
                .sendOutput(new FileQueueMessage(ProductFamily.L1_REPORT,
                        "test.txt", new File("./test/data/report.txt")));
        verify(this.senderProducts, never()).send(Mockito.any());
        verify(this.senderAcns, never()).send(Mockito.any());
        verify(this.senderL1Reports, times(1))
                .send(Mockito.eq(new ReportDto("test.txt", "Test report file",
    ...
    public void testSendL1Product() throws AbstractCodedException {
        this.outputProcuderFactory.sendOutput(new ObsQueueMessage(
                ProductFamily.L1_PRODUCT, "test.txt", "test.txt"));
        verify(this.senderL1Products, times(1))
                .send(Mockito.eq(new L1SliceDto("test.txt", "test.txt")));
        verify(this.senderAcns, never()).send(Mockito.any());
        verify(this.senderL0Reports, never()).send(Mockito.any());
        verify(this.senderProducts, never()).send(Mockito.any());
        verify(this.senderL1Acns, never()).send(Mockito.any());
    ...
    public void testSendL1Acn() throws AbstractCodedException {
        this.outputProcuderFactory.sendOutput(new ObsQueueMessage(
                ProductFamily.L1_ACN, "test.txt", "test.txt"));
        verify(this.senderL1Acns, times(1))
                .send(Mockito.eq(new L1AcnDto("test.txt", "test.txt")));
        verify(this.senderProducts, never()).send(Mockito.any());
        verify(this.senderL0Reports, never()).send(Mockito.any());
        verify(this.senderL1Products, never()).send(Mockito.any());
        verify(this.senderAcns, never()).send(Mockito.any());
    ...
    /**
     * Test send invalid family
     * 
     * @throws AbstractCodedException
     */
    @Test(expected = UnknownFamilyException.class)
    public void testInvalidFamilyForFile() throws AbstractCodedException {
        this.outputProcuderFactory
                .sendOutput(new FileQueueMessage(ProductFamily.JOB, "test.txt",
     ...
     */
    @Test(expected = UnknownFamilyException.class)
    public void testInvalidFamilyForObs() throws AbstractCodedException {
        this.outputProcuderFactory.sendOutput(new ObsQueueMessage(
                ProductFamily.L0_REPORT, "test.txt", "./test/data/report.txt"));
    }
}
